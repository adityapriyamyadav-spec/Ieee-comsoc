...
function AboutUsPage() {
  return (
    <section className="min-h-screen bg-void flex flex-col items-center justify-start px-4 sm:px-6 lg:px-8 py-12 sm:py-16 overflow-hidden">
      ...
      >
        <motion.div variants={itemVariants} whileHover={{ y: -5 }} className="bg-surface border border-trace/30 rounded-2xl p-6 shadow-lg transition-colors duration-300 hover:border-trace/50">
        ...
        <motion.div variants={itemVariants} whileHover={{ y: -5 }} className="bg-surface border border-quantum/30 rounded-2xl p-6 shadow-lg transition-colors duration-300 hover:border-quantum/50">
      ...
      <motion.div 
        ...
        viewport={{ once: true, margin: '-50px' }}
      >
        <motion.h2 variants={itemVariants} className="text-3xl font-bold text-center text-white mb-10">
          Meet the Core Team
        </motion.h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
          {teamMembers.map((member) => (
            <motion.div 
              ...
              whileHover={{ y: -5 }}
              className="bg-surface/50 border border-trace/10 rounded-xl p-5 flex flex-col items-center text-center transition-colors hover:border-trace/40 hover:bg-surface"
            >
              {/* Placeholder Avatar */}
              ...
              <p className="text-xs text-white/60 mt-2 bg-void/50 px-2 py-1 rounded-md">{member.discipline}</p>
            </motion.div>
          ))}
        </div>
      </motion.div>
